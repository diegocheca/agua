...
class Automatico extends CI_Controller {
	...
	public function probando_lista()
	{
		...
		$this->load->view('templates/footer');
		// $this->load->view('templates/footer_listas');
	}
	...
	public function aumentar_orden()
	{
		$resultado = $this->Crud_model->buscar_orden_conexion("A");
		$i=1;
		foreach ($resultado as $key ) {
			$arrayName = array(
				'Conexion_UnionVecinal' => $i);
				$i++;
				echo "Indice:".$i." - Id:".$key->Conexion_Id;
				$resultado = $this->Crud_model->update_data($arrayName, $key->Conexion_Id, "conexion","Conexion_Id");
				echo $resultado." - ";
		}
	}
