...
public function modificar_habilitacion($datos, $id){
	...
	public function get_mediciones_a_aprobar($sector,$mes_buscado,$anio,$limite){
		$this->db->select('Conexion_Categoria,Factura_Id,Factura_Conexion_Id,Factura_Mes,Factura_Año,Factura_MedicionAnterior,Factura_MedicionActual,Factura_Excedentem3,Factura_ExcedentePrecio');
		if( ($sector != false) && ($sector != -1) )
			$this->db->where("conexion.Conexion_Sector", $sector);
		if( ($mes_buscado != false) && ($mes_buscado != -1) )
			$this->db->where("facturacion_nueva.Factura_Mes", $mes_buscado);
		if( ($anio != false) && ($anio != -1) )
			$this->db->where("facturacion_nueva.Factura_Año", $anio);
		if( ($limite != false) && ($limite != -1) )
			$this->db->where("facturacion_nueva.Factura_Excedentem3 >=", $limite);
		$this->db->where("facturacion_nueva.Factura_Borrado", 0);
		$this->db->from("facturacion_nueva");
		$this->db->join('conexion', 'conexion.Conexion_Id = facturacion_nueva.Factura_Conexion_Id'); 
		$consulta = $this->db->get();
			if($consulta->num_rows()>= 1)
