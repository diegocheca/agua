...
<div class="row">
  ...
  <table class="table table-bordered table-striped" style="width:2300px">
	...
	<tbody>
		<tr ng-repeat="factura in facturas | orderBy:sortType:sortReverse | filter:searchFish"  ng-include="getTemplate(factura)">
		</tr>
	</tbody>
		...
		<script type="text/ng-template" id="display">
			<td ng-if="$odd" style="background-color:#f1f1f1" ng-show="mostrar_columna_botones"><button id="boton_mas" ng-click="pagar_boleta($index, factura)" class="btn btn-outline btn-success">$</button><button id="boton_mas" ng-click="crear_pdf(factura)" class="btn btn-outline btn-danger">PDF</button> <button ng-click="editContact(factura)" class="btn btn-outline btn-warning">Edit</button><button ng-click="recalcular_valores($index, factura)" class="btn btn-outline btn-primary">Calcu</button></td>
			<td  ng-if="$even" ng-show="mostrar_columna_botones"><button id="boton_mas" ng-click="pagar_boleta($index, factura)" class="btn btn-outline btn-success">$</button><button id="boton_mas" ng-click="crear_pdf(factura)" class="btn btn-outline btn-danger">PDF</button> <button ng-click="editContact(factura)" class="btn btn-outline btn-warning">Edit</button> <button ng-click="recalcular_valores($index, factura)" class="btn btn-outline btn-primary">Calcu</button></td>
...
//alert ($scope.pago_atrasado);
				...
				$http.post('http://localhost/codeigniter/nuevo/pagar_boleta/'+$scope.selected_dos["id"],  
					...
					success(function(response) {
						if(response !=  true)
							alert("boleta pagada correctamente");
						else 
							{
			...
			$scope.recalcular_boleta = function() {
				...
				$scope.selected_dos.sub_total = $scope.selected_dos.sub_total.toFixed(2);
				$scope.selected_dos.total  = $scope.selected_dos.total.toFixed(2);
				$scope.selected_dos.monto_total_sin_modificar_a  = $scope.selected_dos.monto_total_sin_modificar_a.toFixed(2);
				$scope.pago_parcial();
			}
