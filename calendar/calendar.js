...
$(document).ready(function() {
	...
	$('.mostrador_filtros_informe_tareas').click(function(){
		$('#modal_informe_tareas').modal("show");
	});
	$('.listar_tareas').click(function(){
		$.ajax({
			url:   'tareas/crear_informe_pantalla',
			type:  'post',
			success:  
			function (response) {
				if(response !== false){
					$('#modal_listado_tareas').modal("show");
					$('#resutado_lista_tareas').html(response);
				}
				else{
					if(response == true){
	...
	$('#guadar_evento_nuevo_modal').click(function(){
		...
		var direccion = $('#aclaracion_evento_nuevo').val();
		var inputNombreUsuario = $('#inputNombreUsuario').val();
		var estado = $('#estado_evento_nuevo').val();
		var tecnico = $('#inputTecnico').val();
		var materiales = '';
		var inputObservacion = '';
		$.ajax({
			url: 'http://192.168.1.35/codeigniter/tareas/guardar_evento',
			type: 'POST',
			async: true,
			data: {'inputConexionId': id, 'fecha_inicio_evento_nuevo': fecha_inicio,'fecha_fin_evento_nuevo': fecha_fin,'inputTarea': titulo_evento_nuevo,'inputDireccion': direccion,'estado_evento_nuevo': estado, 'inputNombreUsuario': inputNombreUsuario, 'inputTecnico': tecnico, 'materiales': materiales, 'inputObservacion': inputObservacion},
			success: function(response){
				if(response != false ) // se cargo correctamente
				{
					...
					swal("Orden de Trabajo Creada!", "Orden de Trabajo creada correctamente.", "success"); 
					setTimeout("redireccionarIndex()", 1500);
				}
